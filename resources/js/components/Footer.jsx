      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-6 py-12 md:grid-cols-3">
        <div>
          ...
          <p className="mt-3 text-sm text-white/70">
            {isEn
              ? 'Enjoy a spiritual experience at Pura Lempuyang and the Gate of Heaven panorama with a fast, organized digital booking system.'
        ...
        <div>
          <h4 className="font-semibold text-gold">{isEn ? 'Contact' : 'Kontak'}</h4>
        ...
        <div>
          <h4 className="font-semibold text-gold">{isEn ? 'Visit Us' : 'Kunjungi Kami'}</h4>
          <p className="mt-4 text-sm text-white/80">
            Jalan Pura Telaga Mas, Bunutan, Abang, Karangasem - Bali
          </p>
          <a
          ...
          >
            {isEn ? 'Directions' : 'Navigasi'}
          </a>
        </div>
      </div>
      <div className="border-t border-white/10 py-4 text-center text-xs text-white/60">
        © {year} Pura Lempuyang Luhur — {isEn ? 'An offering for a meaningful journey' : 'Persembahan untuk perjalanan yang bermakna'}
      </div>
    </footer>
  );
};
