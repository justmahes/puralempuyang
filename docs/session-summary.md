...
## Quickstart (TL;DR)
- Copy env and set basics: `cp .env.example .env` (Windows: copy) → set `CACHE_STORE=file`, DB creds, JWT/Midtrans
- Key + migrate + seed + storage: 
...
- Logins: admin `admin@puralempuyang.com/admin123`, operator `operator@puralempuyang.com/operator123`
- Test:
  - Offline mode: `/operator` → scan offline → “Sinkronkan” saat online
  - Antrean foto: user `/dashboard` → “Antrean Foto”; operator `/operator` → pilih titik & tanggal
